...
//func combineLastCharacters(comboArr:[String]) -> String {
func combineLastCharacters(comboArr: Array<String>) -> String{
    var outStr = ""
    for var word in comboArr{
        outStr = "\(outStr)\(word.removeAtIndex(word.endIndex.predecessor()))"
        //in the solutions outStr is appended to rather than added to
    }
    return outStr;
}
...
//is the input string only letters?
func digitsOnly(word:String) ->Bool{
    for charred in word.unicodeScalars{
        if !digits.longCharacterIsMember(charred.value){
            return false
        }
    }
    return true
...
func cleanOnly(dirtArr: Array<String>) -> Array<String>{
    //var outArr = [String]()
    var outArr = Array<String>()
    for word in dirtArr{
...
class MovieArchive {
    func filterByDirector(wantDir:String, inDict: Dictionary<String, String>) -> Array<String>{
        var outArr = Array<String>()
...
var myArch = MovieArchive()
myArch.filterByDirector("Christopher Nolan", inDict: movies)
