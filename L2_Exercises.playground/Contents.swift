...
//useful to avoid key error when retrieving values from a dictionary
if let director = moviesDict[movie]{
    print("Got a \(director)")
...
if let test = number{
    print("\(test) is the magic number.")
}
else{
    print("Didn't get it actually")
}
//: 3b) Change the value of numericalString to "three" and execute the playground again.
//nice, with this example it becomes clear how if let and unpacking can become pretty useful quickly. 
//: ### Exercise 4
...
if let test = artist{
    print("Welcome home \(test)")
}
else{
    print("Couldn't make it happen")
}
//: 5b) Test your code by inserting different values for the variable `painting`.
//Helpful to use the unwrapped feature of optionals when you aren't really sure what you will be casting or converting. Helps affect things on the output a bit.
...
//Implicitly unwrapped optionals are unwrapped automatically, so setting a property should be just fine here
//: ### Exercise 7
...
if let parentVC = childViewController.parentViewController{
    ...
    print("Didn't have one actually")
}
